...
let find_corners ch (row, col) grid =
  ...
  if neighbor_len = 0 then
    4
  else if neighbor_len = 1 then
    2
  else if neighbor_len = 3 then
    find_corners_neighbor_three ch (row, col) neighbors grid
  else if neighbor_len = 4 then
    find_corners_neighbor_four ch (row, col) grid
  else
    find_corners_neighbor_two ch (row, col) neighbors grid
;;

let cal_perimeters_and_area ch (row, col) (grid : char array array) =
  let visited = ref PairSet.empty in
  let rec loop (row, col) =
    if PairSet.mem (row, col) !visited then
      0
    else (
      match get_ele_by_matrix row col grid with
      | None -> 0
...
let result grid =
  ...
  |> Array.iteri (fun row line ->
    ...
    |> Array.iteri (fun col c ->
      if c != '.' then
        total := !total + cal_perimeters_and_area c (row, col) grid));
  !total
;;
...
let () =
  ...
  let list =
    File.read_list_of_line
      (fun line -> line |> String.to_seq |> Array.of_seq)
      file
  in
