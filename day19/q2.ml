...
let make_is_possible (towels : string list) =
  ...
  let rec is_possible design =
    if StringMap.mem design !memo then
      StringMap.find design !memo
    else if design = "" then
      1
    else (
      let res =
        ...
        |> List.fold_left
             (fun acc towel ->
                if String.starts_with design ~prefix:towel then
                  (let design_len = String.length design in
                   let towel_len = String.length towel in
                   is_possible
                     (String.sub design towel_len (design_len - towel_len)))
                  + acc
                else
                  acc)
             0
