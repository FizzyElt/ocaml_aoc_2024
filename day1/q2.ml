...
let parse_line line =
  line |> String.split_on_char ' ' |> List.to_seq
  |> Seq.filter (fun x -> x <> "")
  |> Seq.take 2 |> Seq.map int_of_string |> List.of_seq
...
let split_two_list list =
  let rec loop acc list =
    ...
    | x :: tl ->
        let left, right =
          match x with [ a; b ] -> (a, b) | _ -> failwith "not valid input"
        in
...
let () =
  ...
  |> split_two_list |> result |> print_int
